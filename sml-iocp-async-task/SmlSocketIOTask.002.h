	...
	private:
			...
			case  STEP_READ::START_READ:
			{
				...
				else
				{
					...
					BOOL bgor = ::WSAGetOverlappedResult(
						...
						&_overlappedRead,//_In_   LPWSAOVERLAPPED lpOverlapped,
		...
		START_SWICTH:
			...
			case STEP_WRITE::START_POP:
			{
				...
				_currentTotalWrite = _cbWrite->pop_front(_ioWriteBuffer, _IO_BUFFER_SIZE, /*-1*/50);
				if (_currentTotalWrite == -2)
				{
					_stepWrite = STEP_WRITE::START_POP;
					_taskRunner->StartTask(this, lpoverlapped);
					break;
				}
				else if (_currentTotalWrite < 0)
				{
					//Error(0, nullptr);
				...
				else
				{
					...
					BOOL bgor = ::WSAGetOverlappedResult(
						(SOCKET)_fileShadow->NativeHandle(),//_In_   SOCKET s,
						&_overlappedWrite,//_In_   LPWSAOVERLAPPED lpOverlapped,
						&cbTransfer,//_Out_  LPDWORD lpcbTransfer,
						TRUE,//_In_   BOOL fWait,
						&dwFlags//,//_Out_  LPDWORD lpdwFlags
					);
